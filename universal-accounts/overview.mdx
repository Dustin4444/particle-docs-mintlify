...
The Universal Accounts SDK leverages EIP-7702. As such, if a user creates a Universal Account by connecting an existing wallet (from a supported provider), this wallet and its assets will be automatically upgraded to a Universal Account.
...
<Note>
  Universal Accounts are **independent smart accounts** if used in _smart account mode_, not extensions of an existing wallet.
